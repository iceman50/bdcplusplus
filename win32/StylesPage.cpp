...
showGen(0)
{
	...
	auto add = [this, grouped](tstring&& text, int group, int fontSetting, int textColorSetting, int bgColorSetting) -> Data* {
		auto data = new SettingsData(move(text), fontSetting, textColorSetting, bgColorSetting);
		this->table->insert(grouped ? group : -1, data);
		return data;
	};

	globalData = add(T_("Global application style"), GROUP_GENERAL,
		SettingsManager::MAIN_FONT, SettingsManager::TEXT_COLOR, SettingsManager::BACKGROUND_COLOR);

	add(T_("Uploads"), GROUP_TRANSFERS,
		SettingsManager::UPLOAD_FONT, SettingsManager::UPLOAD_TEXT_COLOR, SettingsManager::UPLOAD_BG_COLOR);
	add(T_("Downloads"), GROUP_TRANSFERS,
		SettingsManager::DOWNLOAD_FONT, SettingsManager::DOWNLOAD_TEXT_COLOR, SettingsManager::DOWNLOAD_BG_COLOR);

	add(T_("Links"), GROUP_CHAT, -1, SettingsManager::LINK_COLOR, -1);
	add(T_("Logs"), GROUP_CHAT, -1, SettingsManager::LOG_COLOR, -1);
...
StylesPage::Data::Data(tstring&& text) :
Flags(),
text(move(text))
{
}
...
StylesPage::SettingsData::SettingsData(tstring&& text, int fontSetting, int textColorSetting, int bgColorSetting) :
Data(move(text)),
