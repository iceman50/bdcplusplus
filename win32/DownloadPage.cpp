...
PropPage(parent, 3, 1)
{
...

	{
		...
		cur2->addChild(Label::Seed(T_("Maximum simultaneous downloads (0 = infinite)")));
		...
		cur2->addChild(Label::Seed(T_("No new downloads if speed exceeds (KiB/s, 0 = disable)")));

		// xgettext:no-c-format
		cur->addChild(Label::Seed(T_("Note; because of changing download speeds, this is not 100% accurate...")));
	}
